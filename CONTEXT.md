...
The content tables carry a nullable text `org_id` (schema v17, PRD #189) — the ownership/scope column delta sync filters on. Server-side, Supabase RLS gates every content row to the caller's org (`org_id IN (my memberships)`), so a device only pulls its own org's rows; client-side the push/pull queries scope on the same key. `active_timers`/`app_settings` are device-local and carry **no** `org_id`. Named `org_id`, not `owner_id`: every user is a **personal org of one** (created by the `handle_new_user` trigger at sign-up), and teams later just insert `memberships` rows pointing several users at a shared org — **zero re-key**, since the column already exists. It is **NULL on the local (sync-off) path** — completely inert, exactly like `deletedAt` — so every shipped build is byte-for-byte as before; first-sign-in **adoption** (`adoptOrphan*` in `lib/data/sync/delta/sync_queries.dart`) stamps the personal `org_id` onto the offline local rows and pushes them when sync is enabled.
